...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Replace widgets via /console/v1/dashboards/{id}/widgets",
    )
    parser.add_argument(
        "--dashboard-id",
        required=True,
    ...
    parser.add_argument(
        "--body-file",
        default=None,
        help="Path to a JSON file containing the replace-widgets body",
    )
    parser.add_argument(
        "--body",
        default=None,
        help="Inline JSON string containing the replace-widgets body",
    )
    parser.add_argument(
        "--endpoint",
...
def parse_payload(payload_text: str) -> dict:
    ...
    if not isinstance(payload, dict):
        raise ValueError("Replace-widgets body must be a JSON object")

    widgets = payload.get("widgets")
    if not isinstance(widgets, list):
        raise ValueError("Replace-widgets body must include `widgets`")
